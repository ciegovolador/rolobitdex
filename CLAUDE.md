...
|-------|--------|-------------|
| **Plan** | `/plan-ceo-review`, `/plan-bitcoiner-review`, `/plan-cypherpunk-review`, `/plan-eng-review`, `/plan-automation-tester-review`, `/plan-design-review`, `/design-consultation` | Before implementation — scope, sovereignty, security, architecture, testing, design |
...
**Full Bitcoin feature** (product + sovereignty + security + architecture + testing + UI):
`/plan-ceo-review` → `/plan-bitcoiner-review` → `/plan-cypherpunk-review` → `/plan-eng-review` → `/plan-automation-tester-review` → `/plan-design-review` → `/opsx:propose` → `/opsx:apply` → `/qa` → `/design-review` → `/ship`
...
**General feature** (product + architecture + testing + UI, no Bitcoin protocol):
`/plan-ceo-review` → `/plan-eng-review` → `/plan-automation-tester-review` → `/plan-design-review` → `/opsx:propose` → `/opsx:apply` → `/qa` → `/design-review` → `/ship`

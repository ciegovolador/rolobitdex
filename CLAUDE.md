...
|-------|--------|-------------|
| **Plan** | `/plan-ceo-review`, `/plan-bitcoiner-review`, `/plan-cypherpunk-review`, `/plan-eng-review`, `/plan-design-review`, `/design-consultation` | Before implementation — scope, sovereignty, security, architecture, design |
...
**Full Bitcoin feature** (product + sovereignty + security + architecture + UI):
`/plan-ceo-review` → `/plan-bitcoiner-review` → `/plan-cypherpunk-review` → `/plan-eng-review` → `/plan-design-review` → `/opsx:propose` → `/opsx:apply` → `/qa` → `/design-review` → `/ship`

**Bitcoin protocol change** (no UI):
`/plan-bitcoiner-review` → `/plan-cypherpunk-review` → `/plan-eng-review` → `/opsx:propose` → `/opsx:apply` → `/review` → `/ship`

**Security-focused change** (key handling, encryption, backup):
`/plan-cypherpunk-review` → `/plan-eng-review` → `/opsx:propose` → `/opsx:apply` → `/review` → `/ship`

**General feature** (product + architecture + UI, no Bitcoin protocol):
